...
import { ModeloProyectos } from './models/Proyectos/Proyectos.js';
import { ObjectId } from 'mongoose';
import { ModeloObjetivos } from './models/Objetivos/Objetivos.js';

// METODOLOGÍA ONE TO MANY #1
const crearProyectoConObjetivos1 = async () => {
  const usuarioInicial = await ModeloUsuarios.create({
    nombre: 'Pepito',
    apellido: 'Pérez',
    correo: 'ppl@pp.com',
    identificacion: '12343210',
  ...
  const objetivoGeneral = await ModeloObjetivos.create({
    descripcion: 'este es el objetivo general',
    tipo: Enum_TipoObjetivo.general,
    proyecto: proyectoCreado._id,
  });

  const objetivoEspecifico1 = await ModeloObjetivos.create({
    descripcion: 'Objetivo Específico 1',
    tipo: Enum_TipoObjetivo.especifico,
    proyecto: proyectoCreado._id,
  });

  const objetivoEspecifico2 = await ModeloObjetivos.create({
...
const consultaProyectoConObjetivos1 = async () => {
  const proyecto = await ModeloProyectos.findOne({ _id: '618d52f71098bc9a121e95d5' });

  console.log('el proyecto que encontré fue', proyecto);

  const objetivos = await ModeloObjetivos.find({ project: '618d52f71098bc9a121e95d5' });
...
const consultaProyectoConObjetivos2 = async () => {
  const proyecto = await ModeloProyectos.find({ id: '618d578f431abaa895d7696d' }).populate(
    'objetivos'
  );
};
...
const consultaProyectoConObjetivos3 = async () => {
  const proyectoCreado = await ModeloProyectos.find({ id: '618d5b22e4e2a99bddab693e' });
...
// await ModeloUsuarios.findOne({ identificacion: '16546' })
//   .then((u) => {
...
// EDITAR UN USUARIO
// await ModeloUsuarios.findOneAndUpdate(
//   { correo: 'dsl@cc.com' },
//   {
//     nombre: 'Juan',
//     apellido: 'López',
...
// ELIMINAR UN USUARIO
// await ModeloUsuarios.findOneAndDelete({ correo: 'dsl@cc.com' })
//   .then((u) => {
